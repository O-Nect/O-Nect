import requests
import time

# generating random coordinates
a = [d/10 for d in range(1, 51, 5)]
b = [d/10 for d in range(3, 53, 5)]
humans = list(zip(a,b))
# [(0.1, 0.3), (0.6, 0.8), (1.1, 1.3), (1.6, 1.8), (2.1, 2.3), (2.6, 2.8), (3.1, 3.3), (3.6, 3.8), (4.1, 4.3), (4.6, 4.8)]

p1_id = 1
p2_id = 2
for x, y in humans:
	params = (p1_id, x, y, p2_id, x+0.5, y+0.5)
	p1_id += 1;	p2_id += 1

	URL = "http://localhost:5000/store?p1_id={}&x1_coord={}&y1_coord={}&p2_id={}&x2_coord={}&y2_coord={}".format(*params)
	#PARAMS = {'xcoord':0.81, 'y_coord':1.89}
	#r = requests.get(url=URL, params = PARAMS)
	requests.get(url=URL)
	print("Sending: "+str(params))
	time.sleep(1)
